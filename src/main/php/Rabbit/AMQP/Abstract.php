...
{
    private static $CONTENT_METHODS =
        array(
            '60,60', // Basic.deliver
            '60,71', // Basic.get_ok
        );

    private static $CLOSE_METHODS =
        array(
            '10,60', // Connection.close
            '20,40', // Channel.close
        );

    // FIXME: Make this private and use a public static method!!
    // All the method names
    public static $GLOBAL_METHOD_NAMES =
        array(
            '10,10' => 'Connection.start', '10,11' => 'Connection.start_ok',
            '10,20' => 'Connection.secure', '10,21' => 'Connection.secure_ok',
            '10,30' => 'Connection.tune', '10,31' => 'Connection.tune_ok',
            '10,40' => 'Connection.open', '10,41' => 'Connection.open_ok',
            '10,50' => 'Connection.redirect', '10,60' => 'Connection.close',
            '10,61' => 'Connection.close_ok', '20,10' => 'Channel.open',
            '20,11' => 'Channel.open_ok', '20,20' => 'Channel.flow',
            '20,21' => 'Channel.flow_ok', '20,30' => 'Channel.alert',
            '20,40' => 'Channel.close', '20,41' => 'Channel.close_ok',
            ...
            '60,10' => 'Channel.basic_qos', '60,11' => 'Channel.basic_qos_ok',
            ...
            '60,60' => 'Channel.basic_deliver', '60,70' => 'Channel.basic_get',
            '60,71' => 'Channel.basic_get_ok',
            '60,72' => 'Channel.basic_get_empty',
            '60,80' => 'Channel.basic_ack', '60,90' => 'Channel.basic_reject',
            '60,100' => 'Channel.basic_recover',
            '90,10' => 'Channel.tx_select', '90,11' => 'Channel.tx_select_ok',
            '90,20' => 'Channel.tx_commit', '90,21' => 'Channel.tx_commit_ok',
    ...
    {
        $this->connection
            ->send_channel_method_frame($this->channel_id, $method_sig, $args);
    }
    ...
    function wait_content()
    {
        ...
        if ($frame_type != 2) {
            // TODO: Use custom exception and constant.
            throw new Exception('Expecting Content header');
        }

        $payload_reader =
            new Rabbit_AMQP_Serialize_Read(substr($payload, 0, 12));
    ...
    public function wait($allowed_methods = null)
    {
        ...
        foreach ($this->method_queue as $qk => $queued_method) {
            ...
            if ($allowed_methods == null
                ...
                return $this
                    ->dispatch($queued_method[0], $queued_method[1],
                        $queued_method[2]);
            }
        }
        while (true) {
